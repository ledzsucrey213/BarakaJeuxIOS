...
class BuyerListViewModel: ObservableObject {
    @Published var searchQuery: String = ""
    @Published var buyers: [User] = []
    @Published var isLoading: Bool = false

    private var cancellables = Set<AnyCancellable>()
    private let apiURL = "http://barakajeuxbackend.cluster-ig4.igpolytech.fr/api/user/role/buyer"
        ...
        URLSession.shared.dataTaskPublisher(for: url)
            ...
            .tryMap { data in
                if let users: [User] = JSONHelper.decode(data: data) {
                    return users.filter { $0.role == .buyer }
                } else {
            ...
            }, receiveValue: { (filteredBuyers: [User]) in
                print("Données reçues : \(filteredBuyers)")  // Affiche les données des utilisateurs récupérées
                self.buyers = filteredBuyers
            })
            .store(in: &cancellables)
    }
 

    var filteredBuyers: [User] {
